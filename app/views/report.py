...
class ReportViewSet(GenericViewSet):
    ...
    def chart_compare_month(self,request):
        ...
        for month in range(1,13):
            serializer = MonthSerializer(month, context={"year":request.query_params.get('year')})
            list_count_by_status = list(serializer.data['count_by_status'].values())
            list_status = tuple(serializer.data['count_by_status'].keys())
            height = list_count_by_status
            bars = list_status
            x_pos = np.arange(len(bars))
            plt.subplot(3, 4, month)
            plt.title(f"%s%s" %("Month", month))
            plt.bar(x_pos, height, color = ['blue', 'coral', 'green', 'red', 'darkgoldenrod', 'gold'])
